...
const SearchableSelect = ({ options = [], value, onChange, placeholder, disabled }) => {
  ...
  return (
    <div className="cs-sel-wrap" ref={ref}>
      ...
      >
        <span className={value ? 'cs-sel-val' : 'cs-sel-ph'}>{value || placeholder}</span>
...
const Customer = () => {
  ...
  const fetchDropdowns = async () => {
    try {
      const [t, co, ca, l] = await Promise.all([
        api.get('/customers/dropdown/types'),
        api.get('/customers/dropdown/countries'),
        api.get('/customers/dropdown/categories'),
        api.get('/customers/dropdown/ltsa-codes')
      ]);
      setTypeOptions(t.data || []);
      setCountryOptions(co.data || []);
      setCategoryOptions(ca.data || []);
      setLtsaOptions(l.data || []);
    } catch {}
  };
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.customer_name, r.Location, r.City, r.customer_country,
       r.buyers, r.customer_type, r.Region, r.Category, r.status]
  ...
  const handleRowDoubleClick = (row) => {
    ...
    if (row.status !== 'Active') {
      showMsg(`Customer "${row.customer_name}" is Inactive and cannot be edited.`, 'warning');
      return;
    }
    setSelected(row);
    setForm({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isAdd && (!form.customer_name || !form.customer_type || !form.customer_country)) {
    ...
    try {
      if (isAdd) {
        ...
        setForm(initForm);
        setAddNewCatg(false);
        await fetchDropdowns(); // refresh category list
      } else if (selected) {
  ...
  const handleToggle = async (row) => {
    ...
    try {
      ...
      showMsg(err.response?.data?.message || 'Failed to change status', 'danger');
    }
  };
    ...
    <div className="cst-page">
      ...
      <div className="cst-body">
        ...
        <div className="cst-breadcrumb">
          <span onClick={() => window.history.back()} className="cst-crumb-link">
        ...
        <div className={`cst-main ${showForm ? 'cst-split' : ''}`}>
          ...
          <div className="cst-table-wrapper">
            <div className="d-flex justify-content-between align-items-center mb-2">
              ...
              <span className="cst-page-info">Page {page} of {totalPages}</span>
            </div>
              ...
              <table className="table table-sm table-hover align-middle cst-table">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={10} className="text-center py-3 text-muted">No records found</td>
                    </tr>
                  ) : (
                    pageRows.map((r, idx) => (
                      <tr
                        ...
                        className={selected?.Sno === r.Sno ? 'cst-row-selected' : ''}
                      >
                        ...
                        <td>
                          {r.buyers
                            ? r.buyers.split(', ').map((b, i) => (
                                <span key={i} className="cst-buyer-tag">{b}</span>
                              ))
                        ...
                        <td className="text-center">
                          {canModify ? (
                            <div
                              className={`cst-toggle ${r.status === 'Active' ? 'cst-toggle-on' : 'cst-toggle-off'}`}
                              onClick={() => handleToggle(r)}
                              title={`Click to set ${r.status === 'Active' ? 'Inactive' : 'Active'}`}
                            >
                              <div className="cst-toggle-thumb"></div>
                              <span className="cst-toggle-label">{r.status}</span>
                            </div>
                          ) : (
          ...
          {showForm && (
            <div className="cst-form-panel">
              <h5 className="cst-form-title">
                <i className={`bi ${isAdd ? 'bi-building-add' : 'bi-pencil-square'} me-2`}></i>
              ...
              <form onSubmit={handleSubmit} noValidate className="cst-form-scroll">

                {/* Customer Name */}
                <div className="mb-2">
                  <label className="form-label">Customer Name <span className="req">*</span></label>
                  <input type="text" className="form-control cst-input" name="customer_name"
                    value={form.customer_name} onChange={handleChange}
                    placeholder="Enter customer name"
                    disabled={!isAdd} required={isAdd} />
                  {!isAdd && <small className="text-muted"><i className="bi bi-lock-fill me-1"></i>Cannot be changed.</small>}
                </div>
                ...
                <div className="mb-2">
                  ...
                  {isAdd ? (
                    ...
                    <>
                      <input type="text" className="form-control cst-input" value={form.customer_type} disabled />
                ...
                <div className="mb-2">
                  ...
                  {isAdd ? (
                    <SearchableSelect
                      ...
                      onChange={v => setForm(p => ({ ...p, customer_country: v }))}
                      placeholder="-- Select Country --"
                    />
                  ) : (
                    <>
                      <input type="text" className="form-control cst-input" value={form.customer_country} disabled />
                      <small className="text-muted"><i className="bi bi-lock-fill me-1"></i>Cannot be changed.</small>
                    </>
                  )}
                </div>
                ...
                <div className="mb-2">
                  ...
                  <input type="text" className="form-control cst-input" name="Location"
                ...
                <div className="mb-2">
                  ...
                  <textarea className="form-control cst-input" name="Address" rows={2}
                    value={form.Address} onChange={handleChange}
                    placeholder="Enter address (optional)" maxLength={250} />
                </div>

                {/* City */}
                <div className="mb-2">
                  <label className="form-label">City</label>
                  <input type="text" className="form-control cst-input" name="City"
                ...
                <div className="mb-2">
                  ...
                  <input type="text" className="form-control cst-input" name="State"
                ...
                <div className="mb-2">
                  ...
                  <input type="text" className="form-control cst-input" name="Region"
                    value={form.Region} onChange={handleChange} placeholder="Enter region" />
                </div>

                {/* Sub Region */}
                <div className="mb-2">
                  <label className="form-label">Sub Region</label>
                  <input type="text" className="form-control cst-input" name="Sub_Region"
                    value={form.Sub_Region} onChange={handleChange} placeholder="Enter sub region" />
                </div>
                ...
                <div className="mb-2">
                  ...
                  {!addNewCatg ? (
                    ...
                    <input type="text" className="form-control cst-input" name="Category"
                      value={form.Category} onChange={handleChange}
                      placeholder="Type new category name" />
                  )}
                  <div className="cst-catg-check mt-1">
                    <input type="checkbox" id="addNewCatg" checked={addNewCatg}
                      onChange={e => { setAddNewCatg(e.target.checked); setForm(p => ({ ...p, Category: '' })); }} />
                ...
                <div className="mb-2">
                  ...
                  <input type="text" className="form-control cst-input" name="Short_name"
                    value={form.Short_name} onChange={handleChange}
                    placeholder="Enter short name" maxLength={90} />
                </div>

                {/* Segment — always INDUSTRIAL, locked */}
                <div className="mb-2">
                  <label className="form-label">Segment</label>
                  <input type="text" className="form-control cst-input" value="INDUSTRIAL" disabled />
                  <small className="text-muted"><i className="bi bi-lock-fill me-1"></i>Fixed as INDUSTRIAL.</small>
                </div>
                ...
                <div className="d-flex gap-2">
                  <button type="submit" className="btn cst-btn-primary flex-fill" disabled={!canModify}>
                    <i className="bi bi-save me-1"></i>{isAdd ? 'Save' : 'Update'}
                  </button>
                  <button type="button" className="btn cst-btn-outline flex-fill" onClick={resetForm}>
